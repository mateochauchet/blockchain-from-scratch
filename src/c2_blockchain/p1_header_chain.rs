...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        ...
        for header in chain {
            // Verificamos que el hash del padre coincida y que la altura sea correcta
...
fn build_an_invalid_chain() -> Vec<Header> {
    let mut chain = vec![Header::genesis()];
    let mut child = chain.last().unwrap().child();
    child.height = 10; // Hacemos inválido el bloque modificando su altura
    chain.push(child);

    let invalid_child = chain.last().unwrap().child();
    chain.push(invalid_child);
